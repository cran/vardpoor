vardchanges <- function(Y1, H1, PSU1, w_final1, id1,
  ...
  if(!is.null(dataset2)) {
      ...
      if(!is.null(Z2)) {
          aZ <- Z2
          if (min(Z2 %in% names(dataset2))!=1) stop("'Z2' does not exist in 'dataset2'!")
          if (min(Z2 %in% names(dataset2))==1) {
                                Z2 <-data.frame(dataset2[, aZ], check.names=FALSE, stringsAsFactors=FALSE)
                                names(Z2) <- aZ }}
      if(!is.null(country2)) {
          acountry <- country2
  ...
  Y1 <- names(Y1)
  dataset1 <- Dom1 <- Z1 <- country1 <- NULL
  ...
  H2 <- names(H2)
  Y2 <- names(Y2)
  PSU2 <- w_final2 <- id2 <- NULL
  dataset2 <- Dom2 <- Z2 <- country2 <- NULL
  ...
  var_grad1[, (c(names(period1), "name2")):=NULL]
  var_grad2[, (c(names(period2), "name2")):=NULL]
  var_grad1[, grad1:=-grad1]
  var_grad1[, grad2:=-grad2]
  setnames(var_grad1, names(var_grad1)[-c(1:2)], paste0(names(var_grad1)[-c(1:2)], "_1"))
  setnames(var_grad2, names(var_grad2)[-c(1:2)], paste0(names(var_grad2)[-c(1:2)], "_2"))
  var_grad1[, country:=as.character(country)]
  var_grad2[, country:=as.character(country)]
  setkeyv(var_grad1, names(var_grad1)[1:2])
  setkeyv(var_grad2, names(var_grad2)[1:2])
  ...
  nrowv <- nrow(var_grad[, .N, by="name1"])
  period <- names(data1)[3+1:nrowv]
  echnames <- names(data1)[(3+nrowv+1):ncol(data1)]
  ...
  echnames <- rep(echnames, nrowv/length(echnames))
  ...
  fit <- lapply(1:nrowv, function(i) {
           ...
           fits <- lapply(split(data, data$country), function(d) {
                   ...
                   if (meanpop) { z1 <- paste0(",", toString(echnames[i]), "_1") 
	                            z2 <- paste0(",", toString(echnames[i]), "_2")
                                } else z1 <- z2 <- ""
                   ...
                   fits <- res[, lapply(.SD, sum), keyby=c("country", "name1"), .SDcols=varsp]
                })
            data.table(do.call("rbind", fits))      
        })
   res <- data.table(do.call("rbind", fit))
   res[, country:=as.character(country)]
   setkeyv(res, c("country", "name1"))
   setkeyv(var_grad, c("country", "name1"))
