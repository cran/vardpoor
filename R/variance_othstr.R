...
variance_othstr <- function(Y, H, H2, w_final, N_h=NULL, N_h2, s2g=FALSE, period=NULL, dataset=NULL) {
  ...
  if (!is.null(N_h2)) {
      ...
      if (ncol(N_h2) != np+2) stop(paste0("'N_h' should be ",toString(np+2)," columns"))
      if (!is.numeric(N_h2[[ncol(N_h2)]])) stop("The last column of 'N_h' should be numerical")
  ...
  zh2 <- z_h_h2[, lapply(.SD, sum, na.rm = T), keyby = c(names(z_h_h2)[2]),
  ...
  # Variance_est
  var_est <- data.table(t(colSums(s2_g, na.rm=T)))
